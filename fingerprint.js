...
const POW_SECRET =
  process.env.POW_SECRET || "fallback-dev-secret-32-chars-minimum";
/**
 ...
 */
export const generateTspChallenge = (
  nonce,
  numCities,
  targetMaxDistance,
  cities,
...
export const verifyTspChallenge = (
  ...
  try {
    ...
    for (let i = 0; i < solutionPath.length; i++) {
      ...
      totalDistance += Optimization.Utils.distance(
        cities[p1_idx],
        cities[p2_idx],
      );
 ...
 */
export const generateCpuPoWChallenge = (
  clientIp,
  nonce,
  difficulty = 4,
  path = "",
 ...
 */
export const generateMemoryPoWChallenge = (
  clientIp,
  nonce,
  difficulty = 16,
  path = "",
 ...
 * @returns {{deviceId: string, isNew: boolean, consistencyScore: number}}
 */
function resolveRequestIdentity(req, res) {
  ...
  if (deviceId && deviceActivityMap.has(deviceId)) {
    // Cas 1: L'utilisateur a un "passeport" et nous le connaissons.
    const data = deviceActivityMap.get(deviceId);
    const storedHash = data.initialDeviceHash;
    ...
    // Si la cohérence est très faible, c'est une alerte.
    // On pourrait même invalider l'ID ici, mais pour l'instant on le signale.
  } else {
    ...
    deviceActivityMap.set(deviceId, {
  ...
  return { deviceId, isNew, consistencyScore };
}

/*
 * Calcule les indicateurs de suspicion liés au comportement de l'IP (historique, rotation).
 * @param {object} req - L'objet de la requête Express.
 * @param {object} data - Les données d'activité de l'IP.
 * @param {number} consistencyScore - Le score de cohérence entre le cookie et l'empreinte.
 * @returns {{historyScore: number, rotationScore: number, inconsistencyScore: number}}
 */
function getBehavioralIndicators(req, data, consistencyScore) {
  const now = Date.now();
  const clientIp = req.ip || req.socket?.remoteAddress || "unknown";

  // On récupère le type d'IP pour moduler le score
  const ipProfile = ipReputationMap.get(clientIp) || { type: "residential" };
  ...
  if (data.lastFpHash && currentFpHash !== data.lastFpHash) {
    const timeSinceLastChange = now - data.lastChangeTimestamp;

    if (timeSinceLastChange < RAPID_CHANGE_THRESHOLD_MS) {
      data.rapidChangeCount = Math.min(
        data.rapidChangeCount + 1,
        MAX_RAPID_CHANGES * 2,
      ); // Augmente rapidement
    } else {
      data.rapidChangeCount = Math.max(0, data.rapidChangeCount - 1); // Diminue lentement
    }
    data.lastChangeTimestamp = now;
  }

  data.lastFpHash = currentFpHash;
  data.ips.add(clientIp); // On enregistre l'IP utilisée par cet appareil
  ...
  // Si l'IP est partagée, on est moins tolérant à la rotation d'IP, car c'est un signe de bot essayant de se cacher dans la foule.
  ...
  const historyScore = Math.min(
    100,
    (Math.max(0, data.ips.size - freeIpChanges) /
      (maxIpsForDevice - freeIpChanges)) *
      100,
  );

  // Score basé sur la rotation rapide d'identité (0-100)
  // Détecte si l'appareil change de User-Agent, etc.
  const rotationScore = Math.min(
    100,
    (data.rapidChangeCount / MAX_RAPID_CHANGES_PER_DEVICE) * 100,
  );
 ...
 * @param {object} req - L'objet de la requête Express.
 * @returns {{historyScore: number, rotationScore: number, headerAnomalyScore: number}}
 */
export const getSuspicionVector = (req, res) => {
  const { deviceId, consistencyScore } = resolveRequestIdentity(req, res);

  const clientIp = req.ip || req.socket?.remoteAddress || "unknown";
  ipToDeviceMap.set(clientIp, deviceId); // On lie l'IP à l'appareil

  const data = deviceActivityMap.get(deviceId);

  // Nettoyage périodique pour cette IP
  if (Date.now() - data.lastUpdate > 10 * 60 * 1000) {
    // 10 minutes
    data.ips.clear();
    data.lastFpHash = null;
    data.rapidChangeCount = 0;
    data.lastUpdate = Date.now();
  }

  const behavioral = getBehavioralIndicators(req, data, consistencyScore);
...
const deviceActivityMap = new Map(); // deviceId -> { initialDeviceHash, ips: Set, ... }
const ipToDeviceMap = new Map(); // ip -> deviceHash (pour le rate-limiter)
// NOUVEAU : Système de réputation d'IP
const ipReputationMap = new Map(); // ip -> { type: 'residential'|'shared', deviceIds: Set, lastSeen: number }
 ...
 */
export const identifyRequest = (req, res) => {
  const clientIp = req.ip || req.socket?.remoteAddress || "unknown";
  const deviceId = req.cookies?.device_id;

  // --- Mise à jour de la réputation de l'IP ---
  const ipProfile = ipReputationMap.get(clientIp) || {
    type: "residential",
    deviceIds: new Set(),
    statelessCount: 0,
    lastSeen: 0,
  ...
  const finalDeviceId = deviceId || ipToDeviceMap.get(clientIp) || clientIp;
...
// On importe la configuration depuis rp.js. C'est un anti-pattern léger (dépendance cyclique),
// mais acceptable pour ce cas précis. Une meilleure solution serait un module de config partagé.
export const optimalChallengeConfig = {
  cpu: {
    minBits: 18,
    maxBits: 26,
  },
...
function calculateTarget(suspicionFactor) {
  ...
  const MIN_DIFFICULTY_BITS = optimalChallengeConfig.cpu.minBits;
  const MAX_DIFFICULTY_BITS = optimalChallengeConfig.cpu.maxBits;
...
export function verifyCpuTargetPoWAndGenerateTicket(
  ...
  if (hashAsInt < target) {
    ...
    const expiry = Date.now() + 3600000; // 1 heure
    const signature = crypto
      .createHmac("sha256", process.env.POW_SECRET)
...
export const powMiddleware = (req, res, next) => {
    ...
    const suspicionVector = getSuspicionVector(req, res);
    const { weights, thresholds } = optimalSecurityConfig;
    ...
    if (isSuspicious && !isTicketValid(clientIp, powCookie)) {
        ...
        if (isSuspicious) {
            ...
            const challengePage = generateChallengePage(challengeDetails, req.path);
